...
##
import time #just for my sake of keeping track of how long this program has to run
...
def decimalToBinaryFixLength(_length, _decimal):
	#print(f"_decimal is {_decimal} with type {type(_decimal)}")
...
def deep_Q_Learning(alpha, gamma, epsilon, episodes, max_steps, n_tests, render = False, test=True,slippery=False):
	"""
	...
	for episode in range(episodes):
		...
		s = env.reset()[0]
		#print(f"s is {s} with type {type(s)} outputted from deep_Q_Learning")
		...
		while t < max_steps:
			...
			# Execute the action 
			s_, reward, terminated, truncated, info = env.step(a) #New API change -- terminated means a complete finish (either finished or failed)
			print(f"reward is {reward}")
			#truncated means the max time has been reached. I this case, we use terminated
			...
			#memory.push(s, a, reward, s_1, done)
			memory.push(s, a, reward, s_, terminated)
...
if __name__ =="__main__":
	...
	max_steps = 5000
	n_tests = 2
	print("Running the Non-slip Version")
	timestep_reward_F, iter_index_F, iter_reward_F, iter_total_steps_F, var_Q_circuit_F, var_Q_bias_F = deep_Q_Learning(alpha, gamma, epsilon, episodes, max_steps, n_tests, test = True,slippery=False)
	t1 = time.time()
	print(f"for the non-slip version, it took {t1-t0} seconds to run")
	timestep_reward, iter_index, iter_reward, iter_total_steps , var_Q_circuit, var_Q_bias = deep_Q_Learning(alpha, gamma, epsilon, episodes, max_steps, n_tests, test = True,slippery=True)
	print(f"ffor the slippery version, it took {time.time()-t1} seconds to run")
	print(f"Time step reward for non slip is {timestep_reward_F}")
	print(f"Time step reward for slippery is {timestep_reward}")
	

	## Drawing Training Result ##
	file_title = 'VQDQN_Frozen_Lake_NonSlip_Dynamic_Epsilon_RMSProp' + datetime.now().strftime("NO%Y%m%d%H%M%S")
	plotTrainingResultReward(_iter_index = iter_index_F, _iter_reward = iter_reward_F, _iter_total_steps = iter_total_steps_F, _fileTitle = 'Quantum_DQN_Frozen_Lake_Nonslip_Dynamic_Epsilon_RMSProp')
	plotTrainingResultReward(_iter_index = iter_index, _iter_reward = iter_reward, _iter_total_steps = iter_total_steps, _fileTitle = 'Quantum_DQN_Frozen_Lake_Slippery_Dynamic_Epsilon_RMSProp')
